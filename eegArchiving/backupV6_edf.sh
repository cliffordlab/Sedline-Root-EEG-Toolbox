...
if [ $(ls /media/pi | wc -l) -eq 1 ]; then 
	...
	if [ "$CHECK" -gt 10737418240 ]; then rm -r /home/pi/Desktop/edf-archive/* || true; fi #10GB limit	
	echo "md5checksum"
	cd ~/Desktop/edf
	for folder in ./*
	do
	...
	sudo rsync -avzh ~/Desktop/edf/. ~/Desktop/edf-archive/. || true
	...
	rclone move ~/Desktop/edf EmoryBox:Sedline\ Data/Emory/EUOSH/edf
	#while [ $? -ne 0 ]; do echo "Trying Again"; rclone move ~/Desktop/edf EmoryBox:Sedline\ Data/Emory/EUOSH/edf ; done
	#sudo rsync -s -avzh --size-only --inplace ~/Desktop/edf/. /home/pi/Box/Sedline\ Data/Emory/EUOSH/${folder1}/${upfolder}/edf ; done
	#sudo umount /home/pi/Box || true
	rm -r ~/Desktop/edf/* || true
